...
function EventPage() {
  ...
  const events = useSelector((state) => state.events); // ini ambil dari reducer events
  const lists = useSelector((state) => state.lists);

  useEffect(() => {
    dispatch(fetchEvents()); // dispatch action fetchEvents
  ...
  // function untuk handle change status, dia nerima id dan status dari TbodyWithAction
  const handleChangeStatus = (id, status) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const payload = {
          statusEvent: status === "Published" ? "Draft" : "Published", // jika status published maka ubah ke draft, jika draft maka ubah ke published
        };
  ...
  return (
    <Container className="mt-3">
      ...
      <Table
        ...
        // customAction dapet id dan status dari TbodyWithAction
