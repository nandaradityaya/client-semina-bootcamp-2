...
export const fetchOrders = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      let params = {
        page: getState().orders?.page || 1, // page yang di terima dari component Table, page = 1 karena kita mau nampilin data dari page 1
        limit: getState().orders?.limit || 10, // limit data per page
        startDate: moment(getState().orders?.date?.startDate).format(
          "YYYY-MM-DD"
        ), // convert dengan moment karena format date yang di terima dari component DateRangePicker berbeda dengan format date yang di terima dari API, klo dari API formatnya YYYY-MM-DD
        endDate: moment(getState().orders?.date?.endDate).format("YYYY-MM-DD"),
      };

      let res = await debouncedFetchOrders("/cms/orders", params); // tiap kali ada perubahan filter date dia akan ngerender ulang
