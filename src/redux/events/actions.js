...
export const fetchEvents = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      // ambil data ini di list action
      let params = {
        keyword: getState().events.keyword,
        category: getState().events?.category?.value || "", // ambil value dari category, ini untuk di pake di react-select
        talent: getState().events?.talent?.value || "",
      };

      let res = await debouncedFetchEvents("/cms/events", params); // pertama kali dijalankan, akan mengirim request ke server
