...
export const fetchListTalents = () => {
  return async (dispatch) => {
    ...
    try {
      ...
      // manipulasi data, jadi ga mentah2 dari API
      res.data.data.forEach((res) => {
        _temp.push({
          value: res._id,
          label: res.name,
          target: { value: res._id, name: "talent" }, // target ini untuk dikirim ke onChange di form, dan name harus sama dengan name di form
        });
      });
